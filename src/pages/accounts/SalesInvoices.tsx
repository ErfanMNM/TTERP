...
export default function SalesInvoices() {
  ...
  const fetchData = () => {
    ...
    const filters = selectedCompany
      ? JSON.stringify([['Sales Invoice', 'company', '=', selectedCompany]])
      : undefined;
    salesInvoiceApi.list({
      filters,
      fields: JSON.stringify(['name', 'customer_name', 'posting_date', 'due_date', 'grand_total', 'outstanding_amount', 'status']),
  ...
  const columns: Array<{ key: string; label: string; sortable?: boolean; minWidth: string; render?: (v: unknown, row: SalesInvoiceRow) => React.ReactNode }> = [
    {
      key: 'name', label: 'Số hóa đơn', sortable: true, minWidth: '120px',
      render: (_: unknown, row: SalesInvoiceRow) => (
        <button onClick={() => navigate(`/selling/sales-invoices/${row.name}`)} className="text-blue-600 hover:text-blue-700 font-medium text-sm">
          {row.name}
        </button>
      ),
    },
    { key: 'customer_name', label: 'Khách hàng', sortable: true, minWidth: '140px' },
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <PageHeader
        ...
        actions={
          <button onClick={() => navigate('/selling/sales-invoices/new')} className="btn btn-primary">
            <Plus size={18} />
            Tạo mới
          </button>
        }
      ...
      <div className="card card-body p-0">
        {loading ? (
          ...
          <DataTable
            ...
            onRowClick={(row: unknown) => navigate(`/selling/sales-invoices/${String((row as Record<string, unknown>).name)}`)}
            emptyText="Chưa có hóa đơn bán"
